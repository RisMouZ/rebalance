...
// TODO UI Autoform. Merge with <MultiSelect />
...
type AutoFormMultiSelectorProps = {
  value?: Option[];
...
export type AutoFormMultiSelectorRef = {
  ...
  (
    ...
    const CreatableItem = () => {
      ...
      if (
        ...
        selected.find(
          (s) => String(s.value).toLowerCase() === inputValue.toLowerCase(),
        )
      ) {
        return undefined;
      }
    ...
    const EmptyItem = React.useCallback(() => {
      if (!creatable && emptyIndicator && (!onSearch || options.length === 0)) {
        return <CommandEmpty>{emptyIndicator}</CommandEmpty>;
      }
      return null;
    }, [creatable, emptyIndicator, onSearch, options.length]);
